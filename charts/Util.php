...
class Util {
  ...
  public static function viewsData(ViewExecutable $view = NULL, array $labelValues = [], $labelField = '', array $color = [], $attachmentChartTypeOption = NULL) {
    ...
    foreach ($view->result as $row_number => $row) {
      ...
      foreach ($labelValues as $fieldId => $rowDataValue) {
        if ($style_options['allow_advanced_rendering'] == 1 || (!empty($view->field[$labelField]->options['type']) && $view->field[$labelField]->options['type'] == 'timestamp')) {
  ...
  //New codes
  /**
   * Get values for first valueField
   */
  public static function getFilters(ViewExecutable $view = NULL, $labelValue) {
    $data = [];
    // $style_options = $view->getStyle()->options;
    foreach ($view->result as $row_number => $row) {
      $data[$view->field[$labelValue]->getValue($row)] = $view->field[$labelValue]->getValue($row);
    }
    return $data;
  }
   ...
   * Creates chart data to be used later by visualization frameworks.
  */
  public static function getFilteredData(ViewExecutable $view = NULL, $labelValue, $filterKey, $filterValue, $color) {
    $data = [];
    foreach ($view->result as $row_number => $row) {
      if ($view->field[$filterKey]->getValue($row) == $filterValue) {
        array_push($data, (float) $view->field[$labelValue]->getValue($row));
      }
    }
    $serie = [
      'name' => self::termNames($filterValue),
      'color' => $color,
      'type' => "line",
      'data' => $data,
    ];
  ...
  /**
   * Get term names for taxo
   */
  public static function termNames($tid) {
